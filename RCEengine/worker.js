...
console.log("RCE Worker started");
...
`,
  ...
  if (!job) {
    console.log("[RCE WORKER] No queued job available");
    return { success: false, message: "No queued job" };
  } else {
    console.log("[RCE WORKER] Claimed job:", job.jobid);
...
console.log("[RCE WORKER] Checking for queued jobs...");
while (true) {
  ...
  if (Jobdata.success) {
    try {
      ...
      console.log("[RCE WORKER] Created directory for job:", Jobdata.jobid);
      fs.writeFileSync(path.join(folderPath, "inputCode.py"), job.inputCode);
      console.log("[RCE WORKER] Wrote inputCode.py for job:", Jobdata.jobid);
      try {
        ...
        console.log(
          "[RCE WORKER] Docker execution completed for job:",
          Jobdata.jobid,
          "Result:",
          resultInputGen,
        );
        ...
        throw Error(
          "[RCE WORKER] Error during input code execution for job:" +
            Jobdata.jobid +
            " Error: " +
            err,
        );
      ...
      console.log("[RCE WORKER] Wrote outputCode.py for job:", Jobdata.jobid);
      try {
        ...
        console.log(
          "[RCE WORKER] Docker execution completed for outputCode.py for job:",
          Jobdata.jobid,
          "Result:",
          resultOutputGen,
        );
        ...
        throw Error(
          "[RCE WORKER] Error during output code execution for job:" +
            Jobdata.jobid +
            " Error: " +
            err,
        );
      ...
      try {
        ...
        throw Error(
          "[RCE WORKER] Error creating markdown file for job:" +
            Jobdata.jobid +
            " Error: " +
            err,
        );
      ...
      console.log("[RCE WORKER] Zipped job folder for job:", Jobdata.jobid);
      // Update job status to completed
      db.prepare("UPDATE jobs SET status=? WHERE jobid=?").run(
        "completed",
        Jobdata.jobid,
      );
      console.log("[RCE WORKER] Completed job:", Jobdata.jobid);
    } catch (err) {
      console.error("[RCE WORKER] Error processing job:", Jobdata.jobid, err);
      db.prepare("UPDATE jobs SET status=? WHERE jobid=?").run(
        "error",
        Jobdata.jobid,
      );
      ...
      console.log("[RCE WORKER] Cleaned up folder for job:", Jobdata.jobid);
    }
  }
  await sleep();
}
