...
router.post('/submit', async (req, res) => {
  ...
  try {
    const testcases = await Prisma.testCase.findMany({   //retrieving all testcases with problemId
      where: { problemId }
    });

    if (testcases.length === 0) throw new Error('Challenge has no testcases');  //if no testcase are present throw error

    const submissions = testcases.map(tc => ({  //create array of submissions for each testcase [{code:...,stdin:...},{}....{}]
      source_code: Code,
    ...
    const judgeRes = await fetch(            //submit in batch all the testcase
      'http://127.0.0.1:2358/submissions/batch?base64_encoded=false&wait=false',
      {
        method: 'POST',
